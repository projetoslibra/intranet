...
function money(value: string) {
  ...
  return cents === null ? "Valor inválido" : formatCentsAsBRL(cents);
}
...
export function ConsignadoDifferenceComposer({
  ...
  function addOtherDifference() {
    onOtherDifferencesChange([...otherDifferences, {
      category: "",
      amount: state.unexplainedCents > BigInt(0) ? centsToDecimalAmount(state.unexplainedCents) : "",
      reason: "",
    }]);
  }
  ...
  return <div className="space-y-4">
      ...
      <div className="border-t border-amber-100 p-4">
        ...
        <div className="mt-4 max-h-80 space-y-2 overflow-auto">
          {visibleExclusions.map((item) => {
            ...
            const paidAmountCents = decimalAmountToCents(item.paidAmount);
            const differenceCents = decimalAmountToCents(difference);
            const exceedsRemaining = !selected && (
              paidAmountCents === null
              || differenceCents === null
              || state.titleTotalCents + paidAmountCents > differenceCents
            );
            const overflowDescriptionId = `exclusion-overflow-${item.id}`;
            return <label className={`block rounded border p-3 ${selected ? "border-primary bg-blue-50" : "border-slate-200"} ${exceedsRemaining ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`} key={item.id}>
              <div className="flex items-start gap-3">
                <input aria-describedby={exceedsRemaining ? overflowDescriptionId : undefined} aria-invalid={exceedsRemaining || undefined} checked={selected} className="mt-1" disabled={disabled || exceedsRemaining} onChange={() => toggleExclusion(item)} type="checkbox" />
                <div className="min-w-0 flex-1">
                  ...
                  {exceedsRemaining ? <p className="mt-1 text-xs font-medium text-red-700" id={overflowDescriptionId}>O valor integral deste título supera o saldo disponível.</p> : null}
                </div>
              </div>
            </label>;
          })}
    ...
    <section className="rounded border border-slate-200 bg-white p-4">
      ...
      {otherDifferences.length ? <div className="mt-4 space-y-3">{otherDifferences.map((item, index) => <div className="grid gap-3 rounded border border-slate-200 p-3 md:grid-cols-[minmax(0,1fr)_10rem_2fr_auto]" key={index}>
        <label className="text-xs font-medium text-slate-600">Categoria<select aria-describedby="difference-composer-errors" aria-invalid={state.hasIncompleteOtherDifference || undefined} className="mt-1 block h-9 w-full rounded border border-slate-200 bg-white px-3 text-sm" disabled={disabled} onChange={(event) => updateOther(index, { category: event.target.value as OtherDifferenceCategory | "" })} value={item.category}><option value="">Selecione</option>{Object.entries(otherCategoryLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        <label className="text-xs font-medium text-slate-600">Valor<input aria-describedby="difference-composer-errors" aria-invalid={state.hasIncompleteOtherDifference || undefined} className="mt-1 block h-9 w-full rounded border border-slate-200 px-3 text-sm" disabled={disabled} inputMode="decimal" onChange={(event) => updateOther(index, { amount: event.target.value })} placeholder="0,00" value={item.amount} /></label>
        <label className="text-xs font-medium text-slate-600">Justificativa<input aria-describedby="difference-composer-errors" aria-invalid={state.hasIncompleteOtherDifference || undefined} className="mt-1 block h-9 w-full rounded border border-slate-200 px-3 text-sm" disabled={disabled} maxLength={500} onChange={(event) => updateOther(index, { reason: event.target.value })} placeholder="Mínimo de cinco caracteres" value={item.reason} /></label>
      ...
      </div>)}</div> : <p className="mt-4 rounded border border-dashed border-slate-200 p-4 text-center text-sm text-slate-500">{state.unexplainedCents > BigInt(0) ? "Adicione um ajuste para explicar o saldo que não foi coberto por títulos." : "Nenhum outro ajuste necessário."}</p>}
    </section>

    <div aria-atomic="true" aria-live="polite" className="text-sm text-slate-700" id="difference-composer-status" role="status">{state.canSubmit ? "Diferença totalmente explicada; conciliação pronta para concluir." : state.unexplainedCents === BigInt(0) ? "Os valores fecham, mas ainda há campos ou seleções pendentes." : state.unexplainedCents > BigInt(0) ? `Ainda falta explicar ${formatCentsAsBRL(state.unexplainedCents)}.` : `A explicação excede a diferença em ${formatCentsAsBRL(-state.unexplainedCents)}.`}</div>
    <div aria-atomic="true" aria-live="assertive" id="difference-composer-errors" role="alert">
      ...
      {state.hasDisallowedTitles || state.hasUnknownSelectedIds || state.hasInvalidSelectedAmount ? <p className="text-sm font-medium text-red-700">A seleção de títulos não está mais disponível. Atualize a composição.</p> : null}
    </div>
  </div>;
}
