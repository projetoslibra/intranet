        ...
        return <details className="p-5" key={batch.id}>
          ...
          <div className="mt-5 space-y-5">
            ...
            {issues.length || pddItems.length ? <section className="rounded border border-amber-200 bg-amber-50/30 p-4">
              ...
              {activeFilter === "NOT_FOUND" ? (notFoundItems.length ? <ConsignadoCrossMatchPanel batchId={batch.id} canManage={canManage} itemIds={notFoundItems.map((item) => item.id)} onApplied={() => refresh()} renderItem={(itemId) => { const found = notFoundItems.find((entry) => entry.id === itemId); return found ? renderIssueItem(batch, found) : null; }} /> : <p className="mt-4 rounded border border-slate-200 bg-white p-4 text-sm text-slate-500">Nenhum título não encontrado.</p>) : null}
