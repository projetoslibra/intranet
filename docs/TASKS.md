...
## 🟢 Concluídas (recentes)
- [x] **[2026-09-23] Dashboard: prazo e taxa média ponderados das aquisições** — APUAMA e BRISTOL passam a congelar, junto ao VOP diário, os componentes de prazo ponderado e taxa de cessão efetiva a.m. ponderada por `valorAquisicao`; snapshots antigos recebem backfill uma única vez sem alterar o VOP, com bloqueio quando a origem histórica diverge. O Dashboard mostra os quatro indicadores na mesma data e a estrutura permite consolidação futura sem média de médias. Exige aplicar a migration `20260923190000_add_vop_operation_indicators` no schema `OSHER` e chamar o cron uma vez após o deploy — _Juan_ · branch `feat/dashboard-operation-indicators`.
- [x] **[2026-09-23] Visibilidade de fundos por módulo** — chaves por fundo na tela Fundos para Dashboard, Caixa, DRE, Previsões e PDD, editáveis somente com `funds.manage`; APUAMA e BRISTOL habilitados em todos, ANTENA somente no Caixa e CONSIGNADO desligado nos cinco. Fundos novos nascem com todas as chaves desligadas. As consultas dos módulos respeitam a configuração e o Caixa também bloqueia gravações individuais ou em lote para fundos não habilitados. Exige a migration `20260923140000_add_fund_module_visibilities` no schema `OSHER` — _Juan_ · branch `feat/fund-module-visibility`.
- [x] **[2026-09-22] Dashboard: snapshots de VOP para APUAMA e BRISTOL** — snapshot diário imutável calculado da posição histórica de `FIDC_ESTOQUES` quando `dataAquisicao = dataReferencia`, backfill autorrecuperável do mês mais recente, janela de estabilidade de 10 minutos, crons às 10h e 15h30 e indicadores de VOP diário/mensal no Dashboard. Nova migration `20260922130000_add_fund_vop_snapshots` deve ser aplicada no schema `OSHER`; após o deploy, executar o endpoint autorizado uma vez para o backfill inicial — _Juan_ · branch `feat/dashboard-vop-snapshots`.
- [x] **[2026-08-31] Reprocessar arquivo de lote excluído (Consignado)** — excluir um lote é exclusão lógica (`status: CANCELLED`), mas a trava de duplicidade por hash do arquivo não filtrava status: o arquivo ficava bloqueado para sempre, e renomear não adianta porque o hash é do conteúdo. Como `cancelSettlementBatch` recusa excluir lote com remessa ativa ou conciliada, um lote excluído nunca efetivou baixa — então ele não deve bloquear. Decisão movida da cláusula `where` para `findBlockingSettlementBatch` no módulo de segurança, com teste. Quando há lote vivo e lote excluído com o mesmo hash, o vivo prevalece — caso real em 3 dos 4 lotes excluídos da base, que continuam bloqueados corretamente por terem gêmeo `RECONCILED`. Sem migration — _Juan_ · branch `fix/reprocessar-lote-excluido`.
- [x] **[2026-08-31] Payload da tela de baixas (Consignado)** — a tela levava ~12s para abrir porque `getSettlementWorkspace` carrega todos os itens dos 30 lotes mais recentes: 20.265 itens, 27,45 MB. Um terço disso era o `sourceRaw`, a linha CNAB crua de 444 caracteres, que o cliente nunca lê (zero usos em `features/`) — 8,58 MB por abertura. Resolvido com `omit: { sourceRaw: true }` só na consulta da tela; a geração de remessa lê o campo pela própria consulta e não foi tocada. Payload 27,45 → 18,58 MB. Medido: a remessa de 6 lotes continua reaproveitando 100% das linhas do `sourceRaw`, com 444 colunas. O tempo cai, mas o quanto não foi possível cravar da máquina local (ruído de rede de 5,5s a 13,3s entre execuções). Fica pendente o conserto maior — resumo dos lotes calculado no servidor e itens buscados só ao expandir, que levaria o payload a ~5 MB — adiado por mexer no painel das ações críticas — _Juan_ · branch `perf/payload-tela-baixas`.
- [x] **[2026-08-31] Acabamento visual da conciliação bancária (Consignado)** — a tela empilhava seis cartões brancos idênticos com subtítulo explicativo em cada um e ignorava a paleta da marca. Cabeçalho passou a ser faixa sem moldura, a barra de totais da seleção ficou escura em `navy-900` com o acento em `emerald`, as listas perderam a sombra pesada, e as frases que só narravam o óbvio saíram. O badge de ajustes `Outro` em aberto foi removido do canto a pedido; como `/diferencas` não tem outra porta de entrada, o link ficou sem valor junto ao histórico. Sem mudança de comportamento — _Juan_ · branch `feat/ui-conciliacao-bancaria`.
- [x] **[2026-08-31] Painel de pendências da conciliação bancária (Consignado)** — a tela mostrava só o badge de ajustes `Outro` em aberto (R$ 997,58) e nunca o valor que de fato importa para o controle: o conciliado sem baixa, R$ 60.612,73 em 22 conciliações. Novo indicador em `consignado-bank-pendencies.ts`, carregado pelo `loadBankReconciliationView` que página e refresh já compartilham, com degradação para "indisponível" em caso de falha isolada. A soma compara entradas contra remessas em vez de agregar `differenceAmount` direto, porque a coluna é absoluta e sozinha misturaria a direção inversa; função pura conferida contra as 32 linhas reais do banco. Tela reorganizada em três blocos, com a barra de totais da seleção fixa no rodapé. Sem migration — _Juan_ · branch `feat/painel-conciliacao-bancaria`.
